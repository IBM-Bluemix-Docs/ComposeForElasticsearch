...
要将备份复原到新服务实例，请执行以下步骤以查看现有备份，然后单击相应的行以展开要复原的备份的选项。单击**复原**按钮。此时将显示一条消息，通知您已启动复原。新服务实例将自动命名为“elasticsearch-restore-[timestamp]”，并在供应启动时显示在仪表板上。

### 通过 {{site.data.keyword.cloud_notm}} CLI 复原

通过 {{site.data.keyword.cloud_notm}} CLI，使用以下步骤以将备份从正在运行的 Elasticsearch 服务复原到新的 Elasticsearch 服务。 
...
```
_SERVICE_ 字段应该为 compose-for-elasticsearch，而 _PLAN_ 字段应该为 Standard 或 Enterprise（取决于环境）。_SERVICE\_INSTANCE\_NAME_ 是放置新服务名称的位置。_source\_service\_instance\_id_ 是备份源的服务实例标识；可通过运行 `bx cf service DISPLAY_NAME --guid` 获取，其中 _DISPLAY\_NAME_ 是备份源自的服务的名称。 
...
例如，将较旧版本的 {{site.data.keyword.composeForElasticsearch}} 服务复原到运行 Elasticsearch 6.2.2 的新服务类似于以下示例：
```
