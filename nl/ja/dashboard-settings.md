...
# 設定
これらの機能を使用して、{{site.data.keyword.composeForElasticsearch_full}} サービスをニーズと要件にさらに適応させることができます。
...
![「バージョン」パネル](./images/elastic_search-version-show.png "「バージョン」パネル")
...
2. _「デプロイメントの詳細 (Deployment Details)」_ペインで、**「リソースのスケール (Scale Resources)」**をクリックします。「リソースのスケール (Scale Resouces)」ページが開きます。
    ...
    ![「リソースの拡大縮小 (Scale Resouces)」パネル](./images/elastic_search-scale-show.png "「リソースの拡大縮小 (Scale Resouces)」パネル")

3. スライダーを調整して、{{site.data.keyword.composeForElasticsearch}} サービスに割り振られたストレージを大きくしたり小さくしたりします。スライダーを左に移動するとストレージ量が減り、右に移動するとストレージが増えます。
4. **「デプロイメントの拡大縮小 (Scale Deployment)」**をクリックしてスケール変更をトリガーし、ダッシュボードの概要に戻ります。_「レシピ (Recipes)」_ペインには、現在進行中のスケール変更ジョブの詳細が含まれます。スケーリングが完了すると、_「デプロイメントの詳細 (Deployment Details)」_ペインが更新され、ストレージの現在の使用量と使用可能な量を示す新しい値が表示されます。
...
![IP のホワイトリスト作成](./images/elastic_search-whitelist-show.png "ホワイトリスト・フィールド。")
