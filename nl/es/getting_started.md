...
## Paso 1: Crear una instancia de servicio de {{site.data.keyword.composeForElasticsearch}}
{: #create-service}
...
4. Actualice el valor `services` para que coincida con el nombre del servicio que ha creado en [Crear una instancia de servicio de {{site.data.keyword.composeForElasticsearch}}](#create-service). 
...
## Paso 8: Comprobar que la app está conectada al servicio de {{site.data.keyword.composeForElasticsearch}}

1. Vaya al panel de control de servicio de {{site.data.keyword.composeForElasticsearch}}
...
Ahora, cuando se visite `<host>.mybluemix.net/`, podrá ver el contenido de la recopilación de {{site.data.keyword.composeForElasticsearch}}. A medida que añada palabras y sus definiciones, se añadirán a la base de datos y se mostrarán. Si detiene y reinicia la app, verá que aparece listada cualquier palabra y definición que ya haya añadido.

## Ejecución de la app localmente

En lugar de enviar la app en {{site.data.keyword.cloud_notm}}, puede ejecutarla localmente para probar la conexión a su instancia de servicio de {{site.data.keyword.composeForElasticsearch}}. Para conectarse al servicio deberá crear un conjunto de credenciales de servicio.

1. Desde el panel de control de {{site.data.keyword.cloud_notm}}, abra la instancia de servicio de {{site.data.keyword.composeForElasticsearch}}.
...
La app se está ejecutando en [http://localhost:8080](http://localhost:8080). Puede añadir palabras y definiciones a la base de datos de {{site.data.keyword.composeForElasticsearch}}. Cuando se detenga y se reinicie la app, cualquier palabra que haya añadido ya se mostrará al renovar la página.

## Pasos siguientes

Para obtener más información sobre cómo funciona la app de ejemplo [compose-elasticsearch-helloworld-nodejs](https://github.com/IBM-Cloud/compose-elasticsearch-helloworld-nodejs), puede leer el archivo readme de la aplicación, o los comentarios de código en `server.js`, que dan alguna información sobre las funciones de la app.

Para empezar a explorar su servicio de {{site.data.keyword.composeForElasticsearch}}, consulte los temas siguientes sobre el panel de control del servicio:
...
Para obtener información sobre las credenciales que ha creado para que la aplicación se conecte al servicio, verá una lista de las [credenciales](/docs/services/ComposeForElasticsearch/connecting-bluemix-app.html#credentials) disponibles.
